...
export const KPI_REGISTRY = {
  assetAvailability: {
    ...
    tone: 'healthy',
    icon: Activity,
    getValue: (ctx) => ctx.summary.assetAvailabilityPct,
  },
  criticalTasks: {
    label: 'Critical tasks',
    tone: 'critical',
    icon: AlertOctagon,
    getValue: (ctx) => ctx.summary.criticalTasks,
  },
  overdueTasks: {
    label: 'Overdue tasks',
    tone: 'warning',
    icon: Clock,
    getValue: (ctx) => ctx.summary.overdueTasks,
  },
  blockUtilization: {
    ...
    tone: 'rail',
    icon: PieChart,
    getValue: (ctx) => ctx.summary.blockUtilizationPct,
  },
  trainConflicts: {
    label: 'Train conflicts',
    tone: 'critical',
    icon: GitMerge,
    getValue: (ctx) => ctx.summary.trainConflicts,
  },
  openTasks: {
    label: 'Open tasks (dept.)',
    tone: 'rail',
    icon: ClipboardList,
    getValue: (ctx) => ctx.summary.departmentTaskCount,
  },
  pendingBlockRequests: {
    label: 'Pending block requests',
    tone: 'warning',
    icon: GitPullRequestArrow,
    getValue: (ctx) => ctx.summary.pendingBlockRequests,
  },
  dataQualityScore: {
